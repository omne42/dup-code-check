...
use crate::util::{
    ...
    fold_u64_to_u32, make_preview, maximal_match, winnowed_fingerprints,
};
...
pub(super) fn detect_duplicate_code_spans(
    ...
    for mut occs in fingerprints.into_values() {
        ...
        if occs.len() > MAX_BUCKET {
            occs = truncate_bucket_by_repo(occs, &normalized, MAX_BUCKET);
        }
        ...
        for i in 0..occs.len() {
            for j in (i + 1)..occs.len() {
                ...
                if options.cross_repo_only
                    && normalized[a.file_id].repo_id == normalized[b.file_id].repo_id
                {
                    continue;
                }
